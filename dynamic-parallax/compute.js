...
const computeExcentricity = (a, b) => Math.sqrt(a ** 2 - b ** 2);
const computeEllipsisArea = (a, b) => Math.PI * a * b;
const computePartialEllipsisArea = (a, b, h) =>
  a * b * (Math.acos(h / a) - (h / a ** 2) * Math.sqrt(a ** 2 - h ** 2));
const computePeriod = (t, S, eps) => (t * S) / eps; // perioda (v rokoch)
const convertAuToParsecs = (au) => au / 206265;
 ...
 */
const velkaPoloosa = (T, M1, M2) => ((M1 + M2) * T ** 2) ** (1 / 3);
 ...
 */
const computeAbsoluteMagnitude = (m, d) => m + 5 - 5 * Math.log10(d);
 ...
 */
const computeLuminosity = (M) => 10 ** ((M - ABS_MAG_SUN) / -2.5) * LUMINOSITY_SUN;
 ...
 */
const computeWeight = (L) => (L / LUMINOSITY_SUN) ** (1 / 3.5);
